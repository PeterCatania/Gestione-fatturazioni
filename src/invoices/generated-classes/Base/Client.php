...
use \ClientCompany as ChildClientCompany;
use \ClientCompanyQuery as ChildClientCompanyQuery;
use \ClientQuery as ChildClientQuery;
use \Invoice as ChildInvoice;
use \InvoiceQuery as ChildInvoiceQuery;
use \Exception;
use \PDO;
use Map\ClientCompanyTableMap;
...
abstract class Client implements ActiveRecordInterface
{
    ...
    /**
     * The value for the house_no field.
     *
     * @var        string
     */
    protected $house_no;
    ...
    /**
     * @var        ObjectCollection|ChildClientCompany[] Collection to store aggregation of ChildClientCompany objects.
     */
    protected $collClientCompanies;
    protected $collClientCompaniesPartial;
     ...
     * @var ObjectCollection|ChildClientCompany[]
     */
    protected $clientCompaniesScheduledForDeletion = null;
    ...
    public function setSurname($v)
    {
        ...
        if ($this->surname !== $v) {
            $this->surname = $v;
            $this->modifiedColumns[ClientTableMap::COL_SURNAME] = true;
        }
    ...
    public function setStreet($v)
    {
        ...
        if ($this->street !== $v) {
            $this->street = $v;
            $this->modifiedColumns[ClientTableMap::COL_STREET] = true;
        }
    ...
    /**
     * Set the value of [house_no] column.
     *
     * @param string $v new value
     * @return $this|\Client The current object (for fluent API support)
     */
    public function setHouseNo($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }
        ...
        if ($this->house_no !== $v) {
            $this->house_no = $v;
            $this->modifiedColumns[ClientTableMap::COL_HOUSE_NO] = true;
        }
    ...
    public function setTelephone($v)
    {
        ...
        if ($this->telephone !== $v) {
            $this->telephone = $v;
            $this->modifiedColumns[ClientTableMap::COL_TELEPHONE] = true;
        }
    ...
    public function setCityId($v)
    {
        ...
        if ($this->city_id !== $v) {
            $this->city_id = $v;
            $this->modifiedColumns[ClientTableMap::COL_CITY_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ClientTableMap::translateFieldName('Surname', TableMap::TYPE_PHPNAME, $indexType)];
            $this->surname = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ClientTableMap::translateFieldName('Street', TableMap::TYPE_PHPNAME, $indexType)];
            $this->street = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : ClientTableMap::translateFieldName('HouseNo', TableMap::TYPE_PHPNAME, $indexType)];
            $this->house_no = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : ClientTableMap::translateFieldName('Telephone', TableMap::TYPE_PHPNAME, $indexType)];
            $this->telephone = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : ClientTableMap::translateFieldName('Email', TableMap::TYPE_PHPNAME, $indexType)];
            $this->email = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : ClientTableMap::translateFieldName('CityId', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->clientCompaniesScheduledForDeletion !== null) {
                if (!$this->clientCompaniesScheduledForDeletion->isEmpty()) {
                    \ClientCompanyQuery::create()
                        ->filterByPrimaryKeys($this->clientCompaniesScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->clientCompaniesScheduledForDeletion = null;
                }
            }

            if ($this->collClientCompanies !== null) {
                foreach ($this->collClientCompanies as $referrerFK) {
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ClientTableMap::COL_SURNAME)) {
            $modifiedColumns[':p' . $index++]  = 'surname';
        }
        if ($this->isColumnModified(ClientTableMap::COL_STREET)) {
            $modifiedColumns[':p' . $index++]  = 'street';
        }
        if ($this->isColumnModified(ClientTableMap::COL_HOUSE_NO)) {
            $modifiedColumns[':p' . $index++]  = 'house_no';
        }
        if ($this->isColumnModified(ClientTableMap::COL_TELEPHONE)) {
            $modifiedColumns[':p' . $index++]  = 'telephone';
        }
        if ($this->isColumnModified(ClientTableMap::COL_EMAIL)) {
            $modifiedColumns[':p' . $index++]  = 'email';
        }
        if ($this->isColumnModified(ClientTableMap::COL_CITY_ID)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'street':
                        ...
                        break;
                    case 'house_no':
                        $stmt->bindValue($identifier, $this->house_no, PDO::PARAM_STR);
                        break;
                    case 'telephone':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collClientCompanies) {
                ...
                $result[$key] = $this->collClientCompanies->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[3], $arr)) {
            $this->setStreet($arr[$keys[3]]);
        }
        if (array_key_exists($keys[4], $arr)) {
            $this->setHouseNo($arr[$keys[4]]);
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ClientTableMap::COL_SURNAME)) {
            $criteria->add(ClientTableMap::COL_SURNAME, $this->surname);
        }
        if ($this->isColumnModified(ClientTableMap::COL_STREET)) {
            $criteria->add(ClientTableMap::COL_STREET, $this->street);
        }
        if ($this->isColumnModified(ClientTableMap::COL_HOUSE_NO)) {
            $criteria->add(ClientTableMap::COL_HOUSE_NO, $this->house_no);
        }
        if ($this->isColumnModified(ClientTableMap::COL_TELEPHONE)) {
            $criteria->add(ClientTableMap::COL_TELEPHONE, $this->telephone);
        }
        if ($this->isColumnModified(ClientTableMap::COL_EMAIL)) {
            $criteria->add(ClientTableMap::COL_EMAIL, $this->email);
        }
        if ($this->isColumnModified(ClientTableMap::COL_CITY_ID)) {
            $criteria->add(ClientTableMap::COL_CITY_ID, $this->city_id);
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setHouseNo($this->getHouseNo());
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getClientCompanies() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addClientCompany($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ($this->aCity === null && ($this->city_id != 0)) {
    ...
    {
        if ('ClientCompany' == $relationName) {
            $this->initClientCompanies();
            return;
        }
    ...
    /**
     * Clears out the collClientCompanies collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return void
     * @see        addClientCompanies()
     */
    public function clearClientCompanies()
    {
        $this->collClientCompanies = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collClientCompanies collection loaded partially.
     */
    public function resetPartialClientCompanies($v = true)
    {
        $this->collClientCompaniesPartial = $v;
    }

    /**
     * Initializes the collClientCompanies collection.
     *
     * By default this just sets the collClientCompanies collection to an empty array (like clearcollClientCompanies());
     ...
     * @return void
     */
    public function initClientCompanies($overrideExisting = true)
    {
        if (null !== $this->collClientCompanies && !$overrideExisting) {
            return;
        }

        $collectionClassName = ClientCompanyTableMap::getTableMap()->getCollectionClassName();

        $this->collClientCompanies = new $collectionClassName;
        $this->collClientCompanies->setModel('\ClientCompany');
    }

    /**
     * Gets an array of ChildClientCompany objects which contain a foreign key that references this object.
     *
     ...
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildClientCompany[] List of ChildClientCompany objects
     * @throws PropelException
     */
    public function getClientCompanies(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collClientCompaniesPartial && !$this->isNew();
        if (null === $this->collClientCompanies || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collClientCompanies) {
                // return empty collection
                $this->initClientCompanies();
            } else {
                $collClientCompanies = ChildClientCompanyQuery::create(null, $criteria)
                    ->filterByClient($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collClientCompaniesPartial && count($collClientCompanies)) {
                        $this->initClientCompanies(false);

                        foreach ($collClientCompanies as $obj) {
                            if (false == $this->collClientCompanies->contains($obj)) {
                                $this->collClientCompanies->append($obj);
                            }
                        }

                        $this->collClientCompaniesPartial = true;
                    }

                    return $collClientCompanies;
                }

                if ($partial && $this->collClientCompanies) {
                    foreach ($this->collClientCompanies as $obj) {
                        if ($obj->isNew()) {
                            $collClientCompanies[] = $obj;
                        }
                    }
                }

                $this->collClientCompanies = $collClientCompanies;
                $this->collClientCompaniesPartial = false;
            }
        }

        return $this->collClientCompanies;
    }

    /**
     * Sets a collection of ChildClientCompany objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $clientCompanies A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildClient The current object (for fluent API support)
     */
    public function setClientCompanies(Collection $clientCompanies, ConnectionInterface $con = null)
    {
        /** @var ChildClientCompany[] $clientCompaniesToDelete */
        $clientCompaniesToDelete = $this->getClientCompanies(new Criteria(), $con)->diff($clientCompanies);


        $this->clientCompaniesScheduledForDeletion = $clientCompaniesToDelete;

        foreach ($clientCompaniesToDelete as $clientCompanyRemoved) {
            $clientCompanyRemoved->setClient(null);
        }

        $this->collClientCompanies = null;
        foreach ($clientCompanies as $clientCompany) {
            $this->addClientCompany($clientCompany);
        }

        $this->collClientCompanies = $clientCompanies;
        $this->collClientCompaniesPartial = false;
    ...
    /**
     * Returns the number of related ClientCompany objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related ClientCompany objects.
     * @throws PropelException
     */
    public function countClientCompanies(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collClientCompaniesPartial && !$this->isNew();
        if (null === $this->collClientCompanies || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collClientCompanies) {
            ...
            if ($partial && !$criteria) {
                return count($this->getClientCompanies());
            }

            $query = ChildClientCompanyQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collClientCompanies);
    }

    /**
     * Method called to associate a ChildClientCompany object to this object
     * through the ChildClientCompany foreign key attribute.
     *
     * @param  ChildClientCompany $l ChildClientCompany
     * @return $this|\Client The current object (for fluent API support)
     */
    public function addClientCompany(ChildClientCompany $l)
    {
        if ($this->collClientCompanies === null) {
            $this->initClientCompanies();
            $this->collClientCompaniesPartial = true;
        ...
        if (!$this->collClientCompanies->contains($l)) {
            $this->doAddClientCompany($l);

            if ($this->clientCompaniesScheduledForDeletion and $this->clientCompaniesScheduledForDeletion->contains($l)) {
                $this->clientCompaniesScheduledForDeletion->remove($this->clientCompaniesScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildClientCompany $clientCompany The ChildClientCompany object to add.
     */
    protected function doAddClientCompany(ChildClientCompany $clientCompany)
    {
        $this->collClientCompanies[]= $clientCompany;
        $clientCompany->setClient($this);
    }

    /**
     * @param  ChildClientCompany $clientCompany The ChildClientCompany object to remove.
     * @return $this|ChildClient The current object (for fluent API support)
     */
    public function removeClientCompany(ChildClientCompany $clientCompany)
    {
        if ($this->getClientCompanies()->contains($clientCompany)) {
            $pos = $this->collClientCompanies->search($clientCompany);
            $this->collClientCompanies->remove($pos);
            if (null === $this->clientCompaniesScheduledForDeletion) {
                $this->clientCompaniesScheduledForDeletion = clone $this->collClientCompanies;
                $this->clientCompaniesScheduledForDeletion->clear();
            }
            $this->clientCompaniesScheduledForDeletion[]= clone $clientCompany;
            $clientCompany->setClient(null);
        }
     ...
     * @return ObjectCollection|ChildInvoice[] List of ChildInvoice objects
     */
    public function getInvoicesJoinTypology(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildInvoiceQuery::create(null, $criteria);
        $query->joinWith('Typology', $joinBehavior);
    ...
    {
        if ($deep) {
            if ($this->collClientCompanies) {
                foreach ($this->collClientCompanies as $o) {
