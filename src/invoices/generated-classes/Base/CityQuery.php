 ...
 * @method     ChildCityQuery orderByNap($order = Criteria::ASC) Order by the nap column
 *
 * @method     ChildCityQuery groupById() Group by the id column
 * @method     ChildCityQuery groupByName() Group by the name column
 * @method     ChildCityQuery groupByNap() Group by the nap column
 *
 ...
 *
 * @method     ChildCityQuery leftJoinClient($relationAlias = null) Adds a LEFT JOIN clause to the query using the Client relation
 * @method     ChildCityQuery rightJoinClient($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Client relation
 * @method     ChildCityQuery innerJoinClient($relationAlias = null) Adds a INNER JOIN clause to the query using the Client relation
 *
 * @method     ChildCityQuery joinWithClient($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Client relation
 *
 * @method     ChildCityQuery leftJoinWithClient() Adds a LEFT JOIN clause and with to the query using the Client relation
 * @method     ChildCityQuery rightJoinWithClient() Adds a RIGHT JOIN clause and with to the query using the Client relation
 * @method     ChildCityQuery innerJoinWithClient() Adds a INNER JOIN clause and with to the query using the Client relation
 *
 * @method     ChildCityQuery leftJoinCompany($relationAlias = null) Adds a LEFT JOIN clause to the query using the Company relation
 * @method     ChildCityQuery rightJoinCompany($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Company relation
 * @method     ChildCityQuery innerJoinCompany($relationAlias = null) Adds a INNER JOIN clause to the query using the Company relation
 *
 * @method     ChildCityQuery joinWithCompany($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Company relation
 *
 * @method     ChildCityQuery leftJoinWithCompany() Adds a LEFT JOIN clause and with to the query using the Company relation
 * @method     ChildCityQuery rightJoinWithCompany() Adds a RIGHT JOIN clause and with to the query using the Company relation
 * @method     ChildCityQuery innerJoinWithCompany() Adds a INNER JOIN clause and with to the query using the Company relation
 *
 * @method     \ClientQuery|\CompanyQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildCity findOneByNap(int $nap) Return the first ChildCity filtered by the nap column *
 ...
 * @method     ChildCity requireOneByNap(int $nap) Return the first ChildCity filtered by the nap column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildCity[]|ObjectCollection findByNap(int $nap) Return ChildCity objects filtered by the nap column
...
abstract class CityQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'invoices', $modelName = '\\City', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, name, nap FROM city WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query on the nap column
     *
     * Example usage:
     * <code>
     * $query->filterByNap(1234); // WHERE nap = 1234
     * $query->filterByNap(array(12, 34)); // WHERE nap IN (12, 34)
     * $query->filterByNap(array('min' => 12)); // WHERE nap > 12
     * </code>
     *
     * @param     mixed $nap The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     ...
     * @return $this|ChildCityQuery The current query, for fluid interface
     */
    public function filterByNap($nap = null, $comparison = null)
    {
        if (is_array($nap)) {
            $useMinMax = false;
            if (isset($nap['min'])) {
                $this->addUsingAlias(CityTableMap::COL_NAP, $nap['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($nap['max'])) {
                $this->addUsingAlias(CityTableMap::COL_NAP, $nap['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(CityTableMap::COL_NAP, $nap, $comparison);
    }

    /**
     * Filter the query by a related \Client object
     *
     * @param \Client|ObjectCollection $client the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
    ...
    public function filterByClient($client, $comparison = null)
    {
        if ($client instanceof \Client) {
            return $this
                ->addUsingAlias(CityTableMap::COL_ID, $client->getCityId(), $comparison);
    ...
    public function filterByCompany($company, $comparison = null)
    {
        if ($company instanceof \Company) {
            return $this
                ->addUsingAlias(CityTableMap::COL_ID, $company->getCityId(), $comparison);
