 ...
 * @method     ChildUser findOneByEnabled(int $enabled) Return the first ChildUser filtered by the enabled column *
 ...
 * @method     ChildUser requireOneByEnabled(int $enabled) Return the first ChildUser filtered by the enabled column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildUser[]|ObjectCollection findByEnabled(int $enabled) Return ChildUser objects filtered by the enabled column
...
abstract class UserQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'invoices', $modelName = '\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, username, password, email, enabled FROM user WHERE id = :p0';
        try {
    ...
    public function filterByEnabled($enabled = null, $comparison = null)
    {
        if (is_array($enabled)) {
            ...
            if (isset($enabled['min'])) {
                $this->addUsingAlias(UserTableMap::COL_ENABLED, $enabled['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($enabled['max'])) {
                $this->addUsingAlias(UserTableMap::COL_ENABLED, $enabled['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
